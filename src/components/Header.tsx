  ...
  return <>
      <header className="sticky top-0 z-50 bg-card/95 backdrop-blur-md border-b border-border" role="banner">
        <div className="container mx-auto px-4">
          <div className="h-16 flex-row flex items-center justify-between rounded shadow-md opacity-100 bg-muted-foreground">
            {/* Logo */}
            ...
            <nav className="hidden lg:flex items-center gap-1" aria-label="Main navigation">
              ...
              <Button variant="ghost" size="sm" className="text-foreground hover:text-primary text-sm" onClick={() => handleNavigation("/ai-resume-builder")}>
                AI Resume
              </Button>
              <Button variant="ghost" size="sm" className="text-foreground hover:text-primary text-sm" onClick={() => handleNavigation("/merge-pdf")}>
                Merge PDF
              </Button>
              <Button variant="ghost" size="sm" className="text-foreground hover:text-primary text-sm" onClick={() => handleNavigation("/compress-pdf")}>
                Compress
              </Button>
            </nav>
          ...
          {/* Desktop Mega Menu */}
          {megaMenuOpen && <div className="hidden lg:block absolute left-0 right-0 top-16 bg-card border-b border-border shadow-xl animate-fade-in z-50" onWheel={e => e.stopPropagation()} onTouchMove={e => e.stopPropagation()}>
        ...
        {megaMenuOpen && <div className="fixed inset-0 top-16 bg-black/20 z-40 hidden lg:block pointer-events-auto" onClick={() => setMegaMenuOpen(false)} aria-hidden="true" />}
      </header>

      {/* Mobile Navigation - OUTSIDE header for proper fixed positioning */}
      {mobileMenuOpen && <div className="lg:hidden fixed inset-0 top-16 bg-card z-[60] overflow-y-auto animate-fade-in overscroll-contain" style={{
      WebkitOverflowScrolling: 'touch'
    }} onWheel={e => e.stopPropagation()} onTouchMove={e => e.stopPropagation()}>
          <MobileMegaMenu onNavigate={closeMobileMenu} />
          <div className="p-4 border-t border-border">
            {user ? <Button variant="outline" className="w-full gap-2" onClick={handleSignOut}>
                <LogOut className="w-4 h-4" />
                Sign out
