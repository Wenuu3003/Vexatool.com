  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">{messages.length} total • {unreadCount} unread</p>
      </div>
            ...
            <Table>
              ...
              <TableBody>
                {messages.map((msg) => (
                  <TableRow
                    key={msg.id}
                    className={`cursor-pointer ${!msg.is_read ? "font-semibold bg-primary/5" : ""}`}
                ...
                {messages.length === 0 && (
                  <TableRow><TableCell colSpan={3} className="text-center py-8 text-muted-foreground">{loading ? "Loading..." : "No messages"}</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </CardContent>
