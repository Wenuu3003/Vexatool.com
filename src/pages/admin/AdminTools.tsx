...
const emptyTool: Omit<Tool, "id"> = {
  ...
  seo_title: "", seo_description: "", seo_keywords: "", category_id: null,
};
...
const AdminTools = () => {
  ...
  const fetchTools = async () => {
    const { data, error } = await supabase.from("admin_tools").select("*").order("sort_order");
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      const { error } = await supabase.from("admin_tools").update(form).eq("id", editing.id);
      if (error) { toast({ title: "Update failed", description: error.message, variant: "destructive" }); return; }
      toast({ title: "Tool updated" });
    } else {
      const { error } = await supabase.from("admin_tools").insert(form);
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await supabase.from("admin_tools").delete().eq("id", id);
    if (error) { toast({ title: "Delete failed", variant: "destructive" }); return; }
    toast({ title: "Tool deleted" });
    fetchTools();
  };

  const handleToggle = async (id: string, field: "is_enabled" | "is_featured", value: boolean) => {
    await supabase.from("admin_tools").update({ [field]: value }).eq("id", id);
    fetchTools();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage all {tools.length} tools</p>
        </div>
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button onClick={openNew}><Plus className="w-4 h-4 mr-2" /> Add Tool</Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input value={form.route_path} onChange={(e) => setForm({ ...form, route_path: e.target.value })} placeholder="/tool-name" />
              </div>
              <div className="space-y-2">
                <Label>Icon</Label>
            ...
            <TableBody>
              {filtered.map((tool) => (
                <TableRow key={tool.id}>
                  ...
                  <TableCell>
                    <Switch checked={tool.is_enabled} onCheckedChange={(v) => handleToggle(tool.id, "is_enabled", v)} />
                  </TableCell>
                  <TableCell>
                    <Switch checked={tool.is_featured} onCheckedChange={(v) => handleToggle(tool.id, "is_featured", v)} />
                  </TableCell>
              ...
              {filtered.length === 0 && (
                <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">{loading ? "Loading..." : "No tools found"}</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
